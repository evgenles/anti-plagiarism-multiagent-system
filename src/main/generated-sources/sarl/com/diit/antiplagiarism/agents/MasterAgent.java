...
public class MasterAgent extends Agent {
  ...
  private void $behaviorUnit$Initialize$0(final Initialize occurrence) {
    ...
    _$CAPACITY_USE$IO_SARL_CORE_LOGGING$CALLER.info(("Master started on " + _GetIp));
  }
  
  private void $behaviorUnit$TaskAddedEvent$1(final TaskAddedEvent occurrence) {
    if ((occurrence.url != null)) {
      String _last = IterableExtensions.<String>last(((Iterable<String>)Conversions.doWrapArray(occurrence.url.split("/"))));
      TaskModel _taskModel = new TaskModel(_last, occurrence.url);
      this.tasks.add(_taskModel);
    }
  }
  ...
  private void $behaviorUnit$ContextJoined$5(final ContextJoined occurrence) {
    ...
    if ((!_isFromMe)) {
      Logging _$CAPACITY_USE$IO_SARL_CORE_LOGGING$CALLER = this.$castSkill(Logging.class, (this.$CAPACITY_USE$IO_SARL_CORE_LOGGING == null || this.$CAPACITY_USE$IO_SARL_CORE_LOGGING.get() == null) ? (this.$CAPACITY_USE$IO_SARL_CORE_LOGGING = this.$getSkill(Logging.class)) : this.$CAPACITY_USE$IO_SARL_CORE_LOGGING);
      Class<? extends ContextJoined> _class = occurrence.getClass();
      Address _source = occurrence.getSource();
      _$CAPACITY_USE$IO_SARL_CORE_LOGGING$CALLER.info(((("Joined: " + _class) + " Address: ") + _source));
    }
  }
  ...
  private void $behaviorUnit$ContextLeft$6(final ContextLeft occurrence) {
    ...
    if ((!_isFromMe)) {
      Logging _$CAPACITY_USE$IO_SARL_CORE_LOGGING$CALLER = this.$castSkill(Logging.class, (this.$CAPACITY_USE$IO_SARL_CORE_LOGGING == null || this.$CAPACITY_USE$IO_SARL_CORE_LOGGING.get() == null) ? (this.$CAPACITY_USE$IO_SARL_CORE_LOGGING = this.$getSkill(Logging.class)) : this.$CAPACITY_USE$IO_SARL_CORE_LOGGING);
      Class<? extends ContextLeft> _class = occurrence.getClass();
      Address _source = occurrence.getSource();
      _$CAPACITY_USE$IO_SARL_CORE_LOGGING$CALLER.info(((("Left: " + _class) + " Address: ") + _source));
    }
  }
  ...
  @SyntheticMember
  @PerceptGuardEvaluator
  private void $guardEvaluator$TaskAddedEvent(final TaskAddedEvent occurrence, final Collection<Runnable> ___SARLlocal_runnableCollection) {
    assert occurrence != null;
    assert ___SARLlocal_runnableCollection != null;
    ___SARLlocal_runnableCollection.add(() -> $behaviorUnit$TaskAddedEvent$1(occurrence));
  }
  ...
  @SyntheticMember
  public MasterAgent(final UUID parentID, final UUID agentID) {
  ...
  @Inject
  @Deprecated
  public MasterAgent(final BuiltinCapacitiesProvider provider, final UUID parentID, final UUID agentID) {
    super(provider, parentID, agentID);
  }
  
  @SyntheticMember
  @Inject
  public MasterAgent(final UUID parentID, final UUID agentID, final DynamicSkillProvider skillProvider) {
